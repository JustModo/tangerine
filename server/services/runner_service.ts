...
export async function runCode(
    ...
    try {
        ...
        if (languageId === "c" || languageId === "cpp") {
            ...
            runCommand = `python3 -u "${codePath}"`; // -u for unbuffered output
        ...
        for (const testCase of testCases) {
            const result = await executeSingleTestCase(runCommand, testCase.input, tempDir);
            ...
            onProgress({
                ...
                executionTime: result.duration ? result.duration.toFixed(2) + "ms" : undefined
            });
        }
...
async function executeSingleTestCase(command: string, input: string, cwd: string): Promise<{ stdout: string; stderr: string; error?: string; isTimeout?: boolean; duration?: number }> {
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
            isTimeout = true;
            child.kill();
            resolve({ stdout, stderr, isTimeout: true, duration: performance.now() - start });
        ...
        child.on("close", (code) => {
            ...
            if (code !== 0) {
                resolve({ stdout, stderr, error: `Process exited with code ${code}`, duration });
            } else {
        ...
        child.on("error", (err) => {
            clearTimeout(timer);
            resolve({ stdout, stderr, error: err.message, duration: performance.now() - start });
        });
    });
}
